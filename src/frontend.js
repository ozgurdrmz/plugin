...
function OurComponent(props) {
  ...
  return (
    <div className="boilerplate-frontend">
      <p>
        <button onClick={() => setShowSkyColor(prev => !prev)}>Toggle frontend sky color</button>
        {showSkyColor && <span>{props.skyColor}</span>}
      </p>
      <p>
        <button onClick={() => setShowGrassColor(prev => !prev)}>Toggle frontend grass color</button>
